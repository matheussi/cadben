namespace MedProj.www
{
    ...
    using cadben.Facade;
    //using cadben.Entidades.Map.NHibernate;
    using FluentNHibernate.Conventions.Helpers;
    //using cadben.Entity.Map.NHibernate;
    ...
    public partial class _default : System.Web.UI.Page
    {
        ...
        {
            IList<cadben.Entidades.Segmento> lista = null;// SegmentoFacade.Instancia.CarregarTodos(string.Empty);
        ...
        void ConfigHome()
        {
            ...
            if (CUtil.UsuarioLogado.TipoUsuario == CUtil.UsuarioLogado.Tipo.Indefinido) //CUtil.UsuarioLogado.Tipo.ContratoDePrestador)
            {
                litPrestador.Text = CUtil.UsuarioLogado.Nome;
                litNomeUnidade.Text = CUtil.UsuarioLogado.NomeUnidade;
                litEnderecoUnidade.Text = CUtil.UsuarioLogado.EnderecoUnidade;

                if (!string.IsNullOrEmpty(CUtil.UsuarioLogado.FoneUnidade))
                    litContatoUnidade.Text = string.Concat("<b>Telefone:</b> ", CUtil.UsuarioLogado.FoneUnidade);

                if (!string.IsNullOrEmpty(CUtil.UsuarioLogado.EmailUnidade))
                {
                    if (!string.IsNullOrEmpty(litContatoUnidade.Text)) litContatoUnidade.Text += "&nbsp;&nbsp;";

                    litContatoUnidade.Text = string.Concat(litContatoUnidade.Text, "<b>E-mail:</b> ", CUtil.UsuarioLogado.EmailUnidade);
                }
            }
            else if (CUtil.UsuarioLogado.TipoUsuario == CUtil.UsuarioLogado.Tipo.Administrador)
            {
                    ...
                    using (IDbCommand cmd = sessao.Connection.CreateCommand())
                    {
                        ...
                        using (IDataReader dr = cmd.ExecuteReader())
                        {
                            while (dr.Read())
                            {
                                ...
                                {
                                    string enderecoTemp = CUtil.CTipos.CToString(row["Endereco"]);
                                    enderecoTemp += " " + CUtil.CTipos.CToString(dr["Complemento"]);
        ...
        protected void cmdBusca_Click(object sender, EventArgs e)
        {
            if (txtBusca.Text.Trim() != "")
            {
                ...
                if (!localizado)
                {
                    CUtil.Geral.Alerta(this, "Nenhum procedimento localizado.");
        ...
        protected void cmdBusca2_Click(object sender, EventArgs e)
        {
            ...
            {
                List<Procedimento> lista = null; //ProcedimentoFacade.Instancia.Carregar(txtBuscaAvancada.Text);
